<div class="card card-primary">
   <div class="card-header">
      <h6 class="card-title"><i class="fas fa-newspaper"></i> Tambah Artikel
      <div class="float-right">
         <a href="<?php echo base_url('paneladmin/artikel') ?>" class="btn btn-xs btn-light"><i class="fa fa-arrow-left"></i> Data Artikel</a>
      </div>
      </h6>
   </div>
	<form method="post" action="<?php echo base_url('artikel/tambah') ?>" enctype="multipart/form-data">
            ...
            <div class="form-group">
               ...
               <select class="form-control select2" name="id_kategori_artikel">
                  ...
                  <?php foreach($kategori->getresult() as $de){
                     echo '<option value="'.$de->id_kategori.'">'.$de->nama_kategori.'</option>';
                  } ?>
               </select>
            </div>
            <div class="form-group">
					...
					<select class="form-control" name="spam_artikel" id="">
						<option value="0">YA Tampilkan</option>
						<option value="1">TIDAK Tampilkan</option>
					</select>
				</div>
            <div class="form-group">
					<label>Tag</label>
					<div class="tags-default">
						<input type="text" class="form-control" name="tag_artikel" value="" data-role="tagsinput" placeholder="Tambah">
					</div>
            </div>
         </div>
   	</div>
   ...
   <div class="card-footer">
      <button class="btn btn-primary" onclick="return confirm('Lanjutkan Simpan Artikel?');"><i class="fa fa-check"></i> Simpan Artikel</button>
...
<?php if(session()->getflashdata('msg')=='simpan'){ ?>
	<script>
	iziToast.show({timeout:5000,color:'green',title: 'Artikel Berhasil Disimpan',position: 'topRight',pauseOnHover: true,transitionIn: false});
</script>
<?php } ?>
